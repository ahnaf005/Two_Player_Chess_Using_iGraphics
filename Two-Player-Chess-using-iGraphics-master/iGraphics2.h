...
extern void LoadBMPWithAlpha(IMAGE *alphaimage, char *filename, int replaceColor=0,int srcR=0,int srcG=0,int srcB=0,int dstR=0,int dstG=0,int dstB=0);
extern void LoadBMPWithAlpha2(IMAGE *image, char *filename);

struct texture
{
    unsigned int  texName ;
    IMAGE img;
    void Create( char *filename,int replaceColor=0,int srcR=0,int srcG=0,int srcB=0,int dstR=0,int dstG=0,int dstB=0);
    void Draw( ) ;
...
void texture::Create( char *filename ,int replaceColor,int srcR,int srcG,int srcB,int dstR,int dstG,int dstB)
{


    ::LoadBMPWithAlpha( &img, filename,replaceColor,srcR,srcG,srcB,dstR,dstG,dstB);
...
void LoadBMPWithAlpha( IMAGE *alphaimage, char *filename, int replaceColor,int srcR,int srcG,int srcB,int dstR,int dstG,int dstB)
{
    AUX_RGBImageRec *Image;
    ...
    for (int src = 0, dst = 0; src < size; src +=3, dst +=4)
    {
        ...
        if (replaceColor== 1&& data[src] ==srcR && data[src+1] == srcG && data[src+2] == srcB){
            alpha[dst] = dstR;
            alpha[dst+1] = dstG;
            alpha[dst+2] = dstB;
...
void LoadBMPWithAlpha2( IMAGE *alphaimage, char *filename)
{
    AUX_RGBImageRec *Image;
